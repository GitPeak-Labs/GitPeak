<script lang="ts">
  import ProfileCard from '$lib/widgets/profile-dashboard/ui/ProfileCard.svelte'
  import StatGrid from '$lib/widgets/profile-dashboard/ui/StatGrid.svelte'
  import LanguagePie from '$lib/widgets/language-breakdown/ui/LanguagePie.svelte'
  import MostStarredRepo from '$lib/widgets/profile-dashboard/ui/MostStarredRepo.svelte'
...
<div class="mx-auto w-full max-w-275">
  <div class="fade-in-up mb-4 flex w-full justify-end [animation-delay:60ms]">
    <a
      href={`/wallpaper/export?username=${encodeURIComponent(username)}`}
      class={cn(
    ...
    >
      <Download size={WALLPAPER_ICON_SIZE} />
      <span>Wallpaper</span>
    </a>
  </div>
